...
class UserRoutes {
    ...
    initRoutes() {
         ...
         * - username: string. It cannot be empty and it must be PPunique (an existing username cannot be used to create a new user)
        ...
        this.router.post(
            ...
            body("role").isString().isIn(["Manager", "Admin", "Customer"]),

            // body("role").isString().matches(/^(Manager|Customer|Admin)$/),
        ...
        this.router.get(
            "/roles/:role",
            this.authenticator.isAdmin,
            param("role").isString().isIn(["Manager", "Admin", "Customer"]),
            this.errorHandler.validateRequest,
         ...
         * It expects the username of the user in the request parameters: the PPusername must represent an existing user.
         * It returns the user.
         */
        this.router.get(
            "/:username",
         ...
         * It expects the username of the user in the request parameters: the username must represent an PPexisting user.
         * It returns a 200 status code.
         */
        this.router.delete(
            "/:username",
        ...
        this.router.patch(
            ...
            body("birthdate").isDate().notEmpty().matches(/^\d{4}-\d{2}-\d{2}$/).isBefore(new Date().toISOString()),
